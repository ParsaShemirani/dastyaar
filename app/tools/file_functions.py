...
def get_current_time():
    ...
    result = dt_now.strftime("%Y-%m-%dT%H:%M:%S") + "Z"
    return result
...
def scp(local_path, remote_user, remote_host, remote_path, upload):
    if upload:
        command = ['scp', local_path, f'{remote_user}@{remote_host}:{remote_path}']
    else:
    ...
    result = subprocess.run(command, capture_output=True, text=True)
    if result.returncode != 0:
        raise Exception(f"SCP command did not work {result.stderr.strip()}")
